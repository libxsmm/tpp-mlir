#!/bin/bash
#
# Correctness test sweep for BF16 GEMM kernels. For every (M, N, K)
# configuration it:
#   1. generates an bf16 GEMM kernel with mlir-gen,
#   2. runs tpp-run with --linalg-to-xsmm (reference implementation),
...
OUT_DIR="${OUT_DIR:-$(pwd)/sfc_ca_gemm_bf16_correctness_results}"
...
for M in "${SIZES[@]}"; do
  for N in "${SIZES[@]}"; do
    for K in "${SIZES[@]}"; do
      ...
      # Run reference implementation (linalg-to-xsmm)
      echo "  Running reference (linalg-to-xsmm)..."
      "${TPP_RUN}" "${mlir_file}" \
        -e entry -entry-point-result=void \
        --init-type=normal --print \
        "${RANDOM_FLAGS[@]}" \
        "${PARALLEL_FLAGS[@]}" \
        > "${ref_out}" 2>&1 || true
      ...
      "${TPP_RUN}" "${mlir_file}" \
        -e entry -entry-point-result=void \
        --vector-to-kernels --registerBlocking=32,32,32 \
        --init-type=normal --print \
        "${RANDOM_FLAGS[@]}" \
        "${PARALLEL_FLAGS[@]}" \
        > "${opt_out}" 2>&1 || true
