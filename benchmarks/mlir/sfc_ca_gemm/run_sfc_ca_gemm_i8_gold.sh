...
# reference for the tpp-run sweep in run_tpp_vector_gemm_32-32-64_i8_f32.sh).
# For every (M, N, K) configuration it:
...
SFC_BIN="${SFC_BIN:-${SCRIPT_DIR}/sfc_ca_gemm-devel_rect_tiles/sfc_ca_gemm}"
# Output directory for the result CSVs.
OUT_DIR="${OUT_DIR:-$(pwd)/sfc_ca_gemm_i8_gold_results}"
...
if [[ ! -x "${SFC_BIN}" ]]; then
  ...
  SFC_SRC_URL="https://github.com/libxsmm/sfc_ca_gemm/archive/refs/heads/devel_rect_tiles.zip"
  SFC_SRC_DIR="${SCRIPT_DIR}/sfc_ca_gemm-devel_rect_tiles"
  SFC_ZIP="${SCRIPT_DIR}/sfc_ca_gemm-devel_rect_tiles.zip"
  ...
  source /swtools/intel/dnnl/2025.3/env/vars.sh

  echo "  Building sfc_ca_gemm (SFC_CA_GEMM_COMPILER=clang make)" >&2
  ( cd "${SFC_SRC_DIR}" && SFC_CA_GEMM_COMPILER=clang make sfc_ca_gemm )
...
mkdir -p "${OUT_DIR}"
RAW_CSV="${OUT_DIR}/sfc_ca_gemm_i8_gold_raw.csv"
...
#==============================================================================
SORTED_CSV="${OUT_DIR}/sfc_ca_gemm_gold_i8_results.csv"
