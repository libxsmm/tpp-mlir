...
// RUN: mlir-gen --output=contract --kernel=const --bias --relu --seed=123 --batch=16 --layers=16,16,16 --data-type=bf16 | tpp-run -e entry -entry-point-result=void
...
// RUN: mlir-gen --output=contract --kernel=const --bias --relu --seed=123 --batch=16 --layers=16,16 --data-type=bf16 | tpp-run -e entry -entry-point-result=void
...
// RUN: mlir-gen --output=contract --kernel=const --bias --relu --seed=123 --batch=16 --layers=16,16 --tiles=8,8,8 --data-type=bf16 | tpp-run -e entry -entry-point-result=void -n 10 | FileCheck %s --check-prefix=PERF
...
// RUN: mlir-gen --output=contract --kernel=const --bias --relu --seed=123 --batch=16 --layers=16,16 --tiles=8,8,8 --data-type=bf16 | tpp-opt --pack-vnni | tpp-run -e entry -entry-point-result=void -n 10 | FileCheck %s --check-prefix=PERF
