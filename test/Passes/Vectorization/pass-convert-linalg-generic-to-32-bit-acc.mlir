...
!tensorA_f8E5M2 = tensor<32x32x8x4xf8E5M2>
!tensorB_f8E5M2 = tensor<32x8x32x4xf8E5M2>
!tensorC_f8E5M2 = tensor<32x32xf8E5M2>

func.func @brgemm_f8E5M2(%arg0: tensor<8x32x32x32xf8E5M2>, %arg1: tensor<32x32x8x32x4xf8E5M2>, %arg2: tensor<8x32x32x32xf8E5M2>) -> tensor<8x32x32x32xf8E5M2> {
  %expanded = tensor.expand_shape %arg0 [[0], [1], [2], [3, 4]] output_shape [8, 32, 32, 8, 4]
                : tensor<8x32x32x32xf8E5M2> into tensor<8x32x32x8x4xf8E5M2>

  %0 = scf.forall (%arg3, %arg4) in (8, 32) shared_outs(%arg5 = %arg2) -> (tensor<8x32x32x32xf8E5M2>) {
    %extracted_slice = tensor.extract_slice %expanded[%arg3, 0, 0, 0, 0] [1, 32, 32, 8, 4] [1, 1, 1, 1, 1]
                : tensor<8x32x32x8x4xf8E5M2> to !tensorA_f8E5M2
    %extracted_slice_0 = tensor.extract_slice %arg1[%arg4, 0, 0, 0, 0] [1, 32, 8, 32, 4] [1, 1, 1, 1, 1]
                : tensor<32x32x8x32x4xf8E5M2> to !tensorB_f8E5M2
    %extracted_slice_1 = tensor.extract_slice %arg5[%arg3, %arg4, 0, 0] [1, 1, 32, 32] [1, 1, 1, 1]
                : tensor<8x32x32x32xf8E5M2> to !tensorC_f8E5M2

    %1 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types =
                ["reduction", "reduction", "parallel", "parallel", "reduction"]}
                ins(%extracted_slice, %extracted_slice_0 : !tensorA_f8E5M2, !tensorB_f8E5M2) outs(%extracted_slice_1 : !tensorC_f8E5M2) {
    ^bb0(%in: f8E5M2, %in_2: f8E5M2, %out: f8E5M2):
      %2 = arith.mulf %in, %in_2 : f8E5M2
      %3 = arith.addf %out, %2 : f8E5M2
      linalg.yield %3 : f8E5M2
    } -> !tensorC_f8E5M2

    scf.forall.in_parallel {
      tensor.parallel_insert_slice %1 into %arg5[%arg3, %arg4, 0, 0] [1, 1, 32, 32] [1, 1, 1, 1]
        : !tensorC_f8E5M2 into tensor<8x32x32x32xf8E5M2>
    }
  }
  return %0 : tensor<8x32x32x32xf8E5M2>
}

// CHECK-LABEL: @brgemm_f8E5M2
// CHECK: tensor.empty() : tensor<32x32xf32>
// CHECK: linalg.fill ins(%cst : f32) outs(%1 : tensor<32x32xf32>) -> tensor<32x32xf32>
// CHECK: linalg.generic {{.*}} ins({{.*}} : tensor<32x32x8x4xf8E5M2>, tensor<32x8x32x4xf8E5M2>) outs({{.*}} : tensor<32x32xf32>) {
// CHECK-NOT: linalg.generic {{.*}} ins({{.*}} : tensor<32x32x8x4xf8E5M2>, tensor<32x8x32x4xf8E5M2>) outs({{.*}} : tensor<32x32xf8E5M2>) {
// CHECK: linalg.generic {{.*}} ins({{.*}} : tensor<32x32xf32>, tensor<32x32xf8E5M2>) outs({{.*}} : tensor<32x32xf8E5M2>) {
...
func.func @batch_matmul_f8E5M2(%arg0: tensor<16x24x32x4xf8E5M2>, %arg1: tensor<16x32x128x4xf8E5M2>, %arg2: tensor<16x24x128xf8E5M2>) -> tensor<16x24x128xf8E5M2> {
  %0 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "reduction", "parallel", "parallel", "reduction"]} ins(%arg0, %arg1 : tensor<16x24x32x4xf8E5M2>, tensor<16x32x128x4xf8E5M2>) outs(%arg2 : tensor<16x24x128xf8E5M2>) {
  ^bb0(%in: f8E5M2, %in_0: f8E5M2, %out: f8E5M2):
    %1 = arith.mulf %in, %in_0 : f8E5M2
    %2 = arith.addf %out, %1 : f8E5M2
    linalg.yield %2 : f8E5M2
  } -> tensor<16x24x128xf8E5M2>
  return %0 : tensor<16x24x128xf8E5M2>
}

// CHECK-LABEL: @batch_matmul_f8E5M2
// CHECK: tensor.empty() : tensor<16x24x128xf32>
// CHECK: linalg.fill ins(%cst : f32) outs(%0 : tensor<16x24x128xf32>) -> tensor<16x24x128xf32>
// CHECK: linalg.generic {{.*}} ins({{.*}} : tensor<16x24x32x4xf8E5M2>, tensor<16x32x128x4xf8E5M2>) outs({{.*}} : tensor<16x24x128xf32>) {
// CHECK-NOT: linalg.generic {{.*}} ins({{.*}} : tensor<16x24x32x4xf8E5M2>, tensor<16x32x128x4xf8E5M2>) outs({{.*}} : tensor<16x24x128xf8E5M2>) {
// CHECK: linalg.generic {{.*}} ins({{.*}} : tensor<16x24x128xf32>, tensor<16x24x128xf8E5M2>) outs({{.*}} : tensor<16x24x128xf8E5M2>) {
...
!tensorA_f8E4M3FN = tensor<32x32x8x4xf8E4M3FN>
!tensorB_f8E4M3FN = tensor<32x8x32x4xf8E4M3FN>
!tensorC_f8E4M3FN = tensor<32x32xf8E4M3FN>

func.func @brgemm_f8E4M3FN(%arg0: tensor<8x32x32x32xf8E4M3FN>, %arg1: tensor<32x32x8x32x4xf8E4M3FN>, %arg2: tensor<8x32x32x32xf8E4M3FN>) -> tensor<8x32x32x32xf8E4M3FN> {
  %expanded = tensor.expand_shape %arg0 [[0], [1], [2], [3, 4]] output_shape [8, 32, 32, 8, 4]
                : tensor<8x32x32x32xf8E4M3FN> into tensor<8x32x32x8x4xf8E4M3FN>

  %0 = scf.forall (%arg3, %arg4) in (8, 32) shared_outs(%arg5 = %arg2) -> (tensor<8x32x32x32xf8E4M3FN>) {
    %extracted_slice = tensor.extract_slice %expanded[%arg3, 0, 0, 0, 0] [1, 32, 32, 8, 4] [1, 1, 1, 1, 1]
                : tensor<8x32x32x8x4xf8E4M3FN> to !tensorA_f8E4M3FN
    %extracted_slice_0 = tensor.extract_slice %arg1[%arg4, 0, 0, 0, 0] [1, 32, 8, 32, 4] [1, 1, 1, 1, 1]
                : tensor<32x32x8x32x4xf8E4M3FN> to !tensorB_f8E4M3FN
    %extracted_slice_1 = tensor.extract_slice %arg5[%arg3, %arg4, 0, 0] [1, 1, 32, 32] [1, 1, 1, 1]
                : tensor<8x32x32x32xf8E4M3FN> to !tensorC_f8E4M3FN

    %1 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types =
                ["reduction", "reduction", "parallel", "parallel", "reduction"]}
                ins(%extracted_slice, %extracted_slice_0 : !tensorA_f8E4M3FN, !tensorB_f8E4M3FN) outs(%extracted_slice_1 : !tensorC_f8E4M3FN) {
    ^bb0(%in: f8E4M3FN, %in_2: f8E4M3FN, %out: f8E4M3FN):
      %2 = arith.mulf %in, %in_2 : f8E4M3FN
      %3 = arith.addf %out, %2 : f8E4M3FN
      linalg.yield %3 : f8E4M3FN
    } -> !tensorC_f8E4M3FN

    scf.forall.in_parallel {
      tensor.parallel_insert_slice %1 into %arg5[%arg3, %arg4, 0, 0] [1, 1, 32, 32] [1, 1, 1, 1]
        : !tensorC_f8E4M3FN into tensor<8x32x32x32xf8E4M3FN>
    }
  }
  return %0 : tensor<8x32x32x32xf8E4M3FN>
}

// CHECK-LABEL: @brgemm_f8E4M3FN
// CHECK: tensor.empty() : tensor<32x32xf32>
// CHECK: linalg.fill ins(%cst : f32) outs(%1 : tensor<32x32xf32>) -> tensor<32x32xf32>
// CHECK: linalg.generic {{.*}} ins({{.*}} : tensor<32x32x8x4xf8E4M3FN>, tensor<32x8x32x4xf8E4M3FN>) outs({{.*}} : tensor<32x32xf32>) {
// CHECK-NOT: linalg.generic {{.*}} ins({{.*}} : tensor<32x32x8x4xf8E4M3FN>, tensor<32x8x32x4xf8E4M3FN>) outs({{.*}} : tensor<32x32xf8E4M3FN>) {
// CHECK: linalg.generic {{.*}} ins({{.*}} : tensor<32x32xf32>, tensor<32x32xf8E4M3FN>) outs({{.*}} : tensor<32x32xf8E4M3FN>) {
...
func.func @batch_matmul_f8E4M3FN(%arg0: tensor<16x24x32x4xf8E4M3FN>, %arg1: tensor<16x32x128x4xf8E4M3FN>, %arg2: tensor<16x24x128xf8E4M3FN>) -> tensor<16x24x128xf8E4M3FN> {
  %0 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "reduction", "parallel", "parallel", "reduction"]} ins(%arg0, %arg1 : tensor<16x24x32x4xf8E4M3FN>, tensor<16x32x128x4xf8E4M3FN>) outs(%arg2 : tensor<16x24x128xf8E4M3FN>) {
  ^bb0(%in: f8E4M3FN, %in_0: f8E4M3FN, %out: f8E4M3FN):
    %1 = arith.mulf %in, %in_0 : f8E4M3FN
    %2 = arith.addf %out, %1 : f8E4M3FN
    linalg.yield %2 : f8E4M3FN
  } -> tensor<16x24x128xf8E4M3FN>
  return %0 : tensor<16x24x128xf8E4M3FN>
}

// CHECK-LABEL: @batch_matmul_f8E4M3FN
// CHECK: tensor.empty() : tensor<16x24x128xf32>
// CHECK: linalg.fill ins(%cst : f32) outs(%0 : tensor<16x24x128xf32>) -> tensor<16x24x128xf32>
// CHECK: linalg.generic {{.*}} ins({{.*}} : tensor<16x24x32x4xf8E4M3FN>, tensor<16x32x128x4xf8E4M3FN>) outs({{.*}} : tensor<16x24x128xf32>) {
// CHECK-NOT: linalg.generic {{.*}} ins({{.*}} : tensor<16x24x32x4xf8E4M3FN>, tensor<16x32x128x4xf8E4M3FN>) outs({{.*}} : tensor<16x24x128xf8E4M3FN>) {
// CHECK: linalg.generic {{.*}} ins({{.*}} : tensor<16x24x128xf32>, tensor<16x24x128xf8E4M3FN>) outs({{.*}} : tensor<16x24x128xf8E4M3FN>) {
