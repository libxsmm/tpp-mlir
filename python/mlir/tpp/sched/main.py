...
from .util import PipelineInterrupt, GpuBackend
...
def config_from_args(args: Sequence[str]) -> Dict[str, Any]:
    ...
    parser = ArgumentParser(prog="tpp-sched.py", description="TODO")
    ...
    parser.add_argument("--enable-amx", action="store_true")
...
def main(args: Sequence[str]) -> ir.Module:
    ...
    if config["input-file"] and config["payload"]:
    ...
    with ir.Context(), ir.Location.name("main_schedule"):
        ...
        if not config["input-file"] and not config["payload"]:
            # If no payload file is provided, just print the schedule.
            print(schedule)
            return

        payload_path = config["input-file"] or config["payload"]

        # If an input file is provided, we load it and apply the schedule to it.
        container = ir.Module.create()
        if payload_path == "-":
            payload = ir.Module.parse(sys.stdin.read())
        else:
            with open(payload_path, "r") as f:
                payload = ir.Module.parse(f.read())
