...
def softmax(inputs: ir.Value, softmax_buf_or_softmax_buf_type: Union[ir.Value, ir.Type]) -> ir.Value:
    ...
    red_affine_map = affine_map(inputs.type.rank, [dims[0], ir.AffineConstantExpr.get(0)])
    ...
    @linalg.generic([exped], [reduction_out], [par_affine_map, red_affine_map], red_iter_types)
    def summed_exped(exped_input, redex):
        return arith.AddFOp(exped_input, redex)

    bcast_out_uninit = tensor.EmptyOp(shape, elem_type)
    @linalg.generic([summed_exped], [bcast_out_uninit.result], [red_affine_map, par_affine_map], par_iter_types)
    def bcasted_summed_exped(input, _output):
        return input

    @linalg.generic([exped, bcasted_summed_exped], [softmax_buf], [par_affine_map] * 3,  par_iter_types)
