...
def floats(shape: Sequence[int], elementType: ir.Type) -> np.ndarray:
...
def get_outputs(outputs_or_outputs_type: Union[ir.Value, ir.Type]) -> ir.Value:
  if isinstance(outputs_or_outputs_type, ir.Value):
    ...
    shape, elem_type = outputs_or_outputs_type.shape, outputs_or_outputs_type.element_type
