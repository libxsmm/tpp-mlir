...
llvm::cl::opt<int64_t> mCachePanel(
    "m-cache-panel",
    llvm::cl::desc("M panel width (register tiles) for k-cache-blocking "
                   "two-level tiling"),
    llvm::cl::init(4));

llvm::cl::opt<int64_t> nCachePanel(
    "n-cache-panel",
    llvm::cl::desc("N panel width (register tiles) for k-cache-blocking "
                   "two-level tiling"),
    llvm::cl::init(1));
...
private:
  ...
  void addDefaultTppPasses() {
    ...
    tppDefaultOptions.mCachePanel = mCachePanel;
    tppDefaultOptions.nCachePanel = nCachePanel;
